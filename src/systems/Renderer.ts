...
const TERRAIN_COLORS: Record<string, { base: number; highlight: number; shadow: number }> = {
    ...
    coniferous_floor: { base: 0x7a9a6a, highlight: 0x8aaa7a, shadow: 0x6a8a5a },
};
    ...
    private drawFoliage(graphics: Graphics, x: number, y: number, foliage: any): void {
        ...
        switch (foliage.foliageType) {
            ...
            case 'pine_bush': {
                ...
                // Needle texture
                for (let i = 0; i < 12; i++) {
                    const nx = x + (seededRandom(i) - 0.5) * 30 * scale;
                    const ny = y - 8 * scale + (seededRandom(i + 10) - 0.5) * 22 * scale;
                    graphics.moveTo(nx - 3 * scale, ny);
                    graphics.lineTo(nx + 3 * scale, ny);
                    graphics.stroke({ color: 0x3a6a3a, width: 2 });
                }
                break;
            }

            case 'pine_tree': {
                // Classic pine with long needles
                graphics.ellipse(x, y + 5, 24 * scale, 10 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });
                ...
                // Triangular crown
                graphics.moveTo(x, y - 70 * scale);
                graphics.lineTo(x - 24 * scale, y - 25 * scale);
                graphics.lineTo(x + 24 * scale, y - 25 * scale);
                graphics.closePath();
                graphics.fill(0x2a5a2a);
                ...
                // Needle clusters
                for (let i = 0; i < 10; i++) {
                    const nx = x + (seededRandom(i) - 0.5) * 40 * scale;
                    const ny = y - 45 * scale + (seededRandom(i + 10) - 0.5) * 35 * scale;
                    graphics.circle(nx, ny, 4 * scale);
                    graphics.fill(seededRandom(i + 20) > 0.5 ? 0x3a6a3a : 0x2a5a2a);
                }
                break;
            }
            ...
            case 'fir_tree': {
                ...
                for (let tier = 0; tier < 4; tier++) {
                    const tierY = y - 20 * scale - tier * 12 * scale;
                    const tierW = (22 - tier * 4) * scale;
                    graphics.moveTo(x, tierY - 10 * scale);
                    graphics.lineTo(x - tierW, tierY + 5 * scale);
                    graphics.lineTo(x + tierW, tierY + 5 * scale);
                    graphics.closePath();
                    graphics.fill(tier % 2 === 0 ? 0x2a5a2a : 0x3a6a3a);
                }
                break;
            }

            case 'chinese_fir': {
                // Elegant with drooping branches
                graphics.ellipse(x, y + 5, 20 * scale, 8 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                // Trunk
                graphics.rect(x - 3 * scale, y - 50 * scale, 6 * scale, 55 * scale);
                graphics.fill(0x6a4a30);

                // Conical crown with drooping effect
                graphics.moveTo(x, y - 65 * scale);
                graphics.quadraticCurveTo(x - 5 * scale, y - 50 * scale, x - 20 * scale, y - 30 * scale);
                graphics.lineTo(x + 20 * scale, y - 30 * scale);
                graphics.quadraticCurveTo(x + 5 * scale, y - 50 * scale, x, y - 65 * scale);
                graphics.fill(0x3a6a3a);

                // Soft needle texture
                for (let i = 0; i < 10; i++) {
                    const nx = x + (seededRandom(i) - 0.5) * 35 * scale;
                    const ny = y - 45 * scale + (seededRandom(i + 10) - 0.5) * 28 * scale;
                    graphics.ellipse(nx, ny, 4 * scale, 2 * scale);
            ...
            case 'yew_tree': {
                ...
                // Thick trunk
                graphics.rect(x - 5 * scale, y - 45 * scale, 10 * scale, 50 * scale);
                graphics.fill(0x5a3a2a);

                // Dense rounded crown
                graphics.ellipse(x, y - 52 * scale, 26 * scale, 20 * scale);
                graphics.fill(0x1a4a1a);

                // Dark foliage clusters
                for (let i = 0; i < 12; i++) {
                    const nx = x + (seededRandom(i) - 0.5) * 46 * scale;
                    const ny = y - 52 * scale + (seededRandom(i + 10) - 0.5) * 34 * scale;
                    graphics.ellipse(nx, ny, 5 * scale, 3 * scale);
                    graphics.fill(seededRandom(i + 20) > 0.5 ? 0x2a5a2a : 0x1a4a1a);
                }

                // Red berries occasionally
                if (seededRandom(999) > 0.5) {
                    for (let i = 0; i < 5; i++) {
                        const bx = x + (seededRandom(i + 400) - 0.5) * 30 * scale;
                        const by = y - 48 * scale + seededRandom(i + 410) * 15 * scale;
            ...
            case 'dawn_redwood': {
                // Tall with feathery foliage and reddish bark
                graphics.ellipse(x, y + 6, 26 * scale, 11 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                // Reddish trunk
                graphics.rect(x - 5 * scale, y - 60 * scale, 10 * scale, 66 * scale);
                graphics.fill(0x8a4a3a);

                // Conical feathery crown
                graphics.moveTo(x, y - 80 * scale);
                graphics.lineTo(x - 22 * scale, y - 35 * scale);
                graphics.lineTo(x + 22 * scale, y - 35 * scale);
                graphics.closePath();
                graphics.fill(0x4a7a4a);
                ...
                // Feathery texture
                for (let i = 0; i < 14; i++) {
                    const nx = x + (seededRandom(i) - 0.5) * 38 * scale;
                    const ny = y - 55 * scale + (seededRandom(i + 10) - 0.5) * 35 * scale;
                    graphics.ellipse(nx, ny, 3 * scale, 1.5 * scale);
            ...
            case 'yellow_cedar': {
                // Drooping branches with yellowish tinge
                graphics.ellipse(x, y + 5, 24 * scale, 10 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                // Trunk
                graphics.rect(x - 4 * scale, y - 55 * scale, 8 * scale, 60 * scale);
                graphics.fill(0x7a5a3a);

                // Conical crown
                graphics.moveTo(x, y - 72 * scale);
                graphics.lineTo(x - 20 * scale, y - 30 * scale);
                graphics.lineTo(x + 20 * scale, y - 30 * scale);
                graphics.closePath();
                graphics.fill(0x5a7a3a);

                // Drooping branch tips
                for (let i = 0; i < 8; i++) {
                    const bx = x + (seededRandom(i) - 0.5) * 35 * scale;
                    const by = y - 50 * scale + seededRandom(i + 10) * 20 * scale;
                    graphics.moveTo(bx, by);
                    graphics.quadraticCurveTo(bx, by + 8 * scale, bx + (seededRandom(i + 20) - 0.5) * 6 * scale, by + 12 * scale);
                    graphics.stroke({ color: 0x6a8a4a, width: 2 });
                }
                break;
            }
            ...
            case 'spruce_tree': {
                ...
                // Trunk
                graphics.rect(x - 3 * scale, y - 60 * scale, 6 * scale, 65 * scale);
                graphics.fill(0x5a4030);

                // Narrow conical crown
                graphics.moveTo(x, y - 78 * scale);
                graphics.lineTo(x - 16 * scale, y - 25 * scale);
                graphics.lineTo(x + 16 * scale, y - 25 * scale);
                graphics.closePath();
                graphics.fill(0x2a5a3a);

                // Horizontal branch layers
                for (let i = 0; i < 6; i++) {
                    const by = y - 30 * scale - i * 8 * scale;
                    const bw = (14 - i * 2) * scale;
                    graphics.moveTo(x - bw, by);
                    graphics.lineTo(x + bw, by);
                    graphics.stroke({ color: 0x3a6a4a, width: 3 });
                }
                break;
            }
            ...
            case 'lodgepole_pine': {
                ...
                // Tall thin trunk
                graphics.rect(x - 2.5 * scale, y - 65 * scale, 5 * scale, 69 * scale);
                graphics.fill(0x6a5040);

                // Narrow crown at top
                graphics.ellipse(x, y - 70 * scale, 14 * scale, 18 * scale);
                graphics.fill(0x3a6a3a);

                // Sparse needle clusters
                for (let i = 0; i < 8; i++) {
                    const nx = x + (seededRandom(i) - 0.5) * 24 * scale;
                    const ny = y - 70 * scale + (seededRandom(i + 10) - 0.5) * 30 * scale;
                    graphics.circle(nx, ny, 3 * scale);
                    graphics.fill(0x4a7a4a);
                }
                break;
            }

            case 'western_red_cedar': {
                // Majestic cedar with scale-like foliage
                graphics.ellipse(x, y + 6, 28 * scale, 12 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                // Thick reddish trunk
                graphics.rect(x - 6 * scale, y - 58 * scale, 12 * scale, 64 * scale);
                graphics.fill(0x7a4030);

                // Wide conical crown
                graphics.moveTo(x, y - 78 * scale);
                graphics.lineTo(x - 28 * scale, y - 30 * scale);
                graphics.lineTo(x + 28 * scale, y - 30 * scale);
                graphics.closePath();
                graphics.fill(0x2a5a2a);

                // Scale-like foliage sprays
                for (let i = 0; i < 12; i++) {
                    const sx = x + (seededRandom(i) - 0.5) * 48 * scale;
                    const sy = y - 52 * scale + (seededRandom(i + 10) - 0.5) * 38 * scale;
                    graphics.ellipse(sx, sy, 6 * scale, 3 * scale);
                    graphics.fill(seededRandom(i + 20) > 0.5 ? 0x3a6a3a : 0x4a7a4a);
                }
                break;
            }

            case 'norfolk_island_pine': {
                // Very symmetrical tiers
                graphics.ellipse(x, y + 5, 22 * scale, 9 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });
                ...
                // Symmetrical whorled branches
                for (let tier = 0; tier < 6; tier++) {
                    const tierY = y - 20 * scale - tier * 8 * scale;
                    const tierW = (20 - tier * 2.5) * scale;
                    // Horizontal branches
                    graphics.moveTo(x - tierW, tierY);
                    graphics.lineTo(x + tierW, tierY);
                    graphics.stroke({ color: 0x3a6a3a, width: 3 });
                    // Upturned tips
                    graphics.moveTo(x - tierW, tierY);
                    graphics.lineTo(x - tierW + 3 * scale, tierY - 4 * scale);
                    graphics.stroke({ color: 0x4a7a4a, width: 2 });
                    graphics.moveTo(x + tierW, tierY);
                    graphics.lineTo(x + tierW - 3 * scale, tierY - 4 * scale);
                    graphics.stroke({ color: 0x4a7a4a, width: 2 });
                }
                ...
                // Top spike
                graphics.moveTo(x, y - 68 * scale);
                graphics.lineTo(x, y - 75 * scale);
                graphics.stroke({ color: 0x3a6a3a, width: 3 });
                break;
            }

            case 'club_moss': {
                // Low prehistoric ground cover
                graphics.ellipse(x, y + 2, 18 * scale, 7 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.15 });

                // Dense low mound
                graphics.ellipse(x, y - 6 * scale, 20 * scale, 12 * scale);
                graphics.fill(0x3a6a3a);

                // Scale-like leaves radiating out
                for (let i = 0; i < 16; i++) {
                    const angle = (i / 16) * Math.PI * 2;
                    const dist = 8 + seededRandom(i) * 8;
                    const mx = x + Math.cos(angle) * dist * scale;
                    const my = y - 6 * scale + Math.sin(angle) * dist * 0.5 * scale;
                    graphics.ellipse(mx, my, 3 * scale, 2 * scale);
                    graphics.fill(seededRandom(i + 20) > 0.5 ? 0x4a7a4a : 0x5a8a5a);
                }

                // Spore-bearing cones occasionally
                if (seededRandom(888) > 0.6) {
                    for (let i = 0; i < 3; i++) {
                        const cx = x + (seededRandom(i + 300) - 0.5) * 16 * scale;
                        graphics.rect(cx - 1 * scale, y - 12 * scale - seededRandom(i + 310) * 6 * scale, 2 * scale, 8 * scale);
                        graphics.fill(0x6a8a5a);
                    }
                }
                break;
            }

            case 'walchian_conifer': {
                // Ancient Jurassic conifer
                graphics.ellipse(x, y + 5, 22 * scale, 9 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });
                ...
                // Strap-like leaves
                for (let i = 0; i < 10; i++) {
                    const lx = x + (seededRandom(i) - 0.5) * 35 * scale;
                    const ly = y - 50 * scale + (seededRandom(i + 10) - 0.5) * 32 * scale;
                    graphics.ellipse(lx, ly, 5 * scale, 2 * scale);
            ...
            case 'lepidodendron': {
                // Giant scale tree - palm-like prehistoric
                graphics.ellipse(x, y + 6, 24 * scale, 10 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                // Tall trunk with diamond scale pattern
                const lepH = 65 * scale;
                graphics.rect(x - 5 * scale, y - lepH, 10 * scale, lepH);
                graphics.fill(0x5a5a4a);

                // Diamond scale bark pattern
                for (let row = 0; row < 10; row++) {
                    for (let col = 0; col < 2; col++) {
                        const sx = x - 3 * scale + col * 6 * scale;
                        const sy = y - 5 * scale - row * 6 * scale;
                        graphics.moveTo(sx + 2 * scale, sy);
                        graphics.lineTo(sx, sy + 2 * scale);
                        graphics.lineTo(sx + 2 * scale, sy + 4 * scale);
                        graphics.lineTo(sx + 4 * scale, sy + 2 * scale);
                        graphics.closePath();
                        graphics.stroke({ color: 0x4a4a3a, width: 0.5 });
                    }
                }

                // Tuft of grass-like leaves at top
                for (let i = 0; i < 12; i++) {
                    const angle = (i / 12) * Math.PI * 2;
                    const leafLen = 20 * scale;
                    const endX = x + Math.cos(angle) * leafLen;
                    const endY = y - lepH - 5 + Math.sin(angle) * leafLen * 0.3;
                    graphics.moveTo(x, y - lepH);
                    graphics.quadraticCurveTo(
                        x + Math.cos(angle) * leafLen * 0.6,
                        y - lepH - 3 + Math.sin(angle) * leafLen * 0.2,
                        endX, endY
                    );
                    graphics.stroke({ color: 0x5a7a4a, width: 2 * scale });
                }
                break;
            }
            ...
            case 'sand_bush': {
                ...
                // Grayish-green desert foliage
                graphics.circle(x, y - 6 * scale, 14 * scale);
                graphics.fill(0x7a8a6a);
                graphics.circle(x - 5 * scale, y - 10 * scale, 10 * scale);
                graphics.fill(0x8a9a7a);
                graphics.circle(x + 6 * scale, y - 8 * scale, 9 * scale);
                graphics.fill(0x7a8a6a);

                // Small branches showing through
                for (let i = 0; i < 4; i++) {
                    const bx = x - 6 * scale + i * 4 * scale;
                    graphics.moveTo(bx, y);
                    graphics.lineTo(bx + (seededRandom(foliage.id + i) - 0.5) * 4, y - 12 * scale);
                    graphics.stroke({ color: 0x6a5a4a, width: 1 });
                }
                break;
            }

            case 'agave_cactus': {
                // Succulent rosette plant
                graphics.ellipse(x, y + 4, 18 * scale, 8 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                // Rosette of thick pointed leaves
                for (let i = 0; i < 12; i++) {
                    const angle = (i / 12) * Math.PI * 2 + foliage.rotation;
                    const leafLen = 18 * scale;
                    const endX = x + Math.cos(angle) * leafLen;
                    const endY = y - 8 * scale + Math.sin(angle) * leafLen * 0.4;

                    // Thick succulent leaf
                    graphics.moveTo(x, y - 5 * scale);
                    graphics.lineTo(endX - 2 * scale, endY);
                    graphics.lineTo(endX, endY - 3 * scale);
                    graphics.lineTo(endX + 2 * scale, endY);
                    graphics.closePath();
                    graphics.fill(0x5a7a5a);
                }

                // Central flower stalk (sometimes)
                if (seededRandom(foliage.id) > 0.7) {
                    graphics.rect(x - 1 * scale, y - 35 * scale, 2 * scale, 30 * scale);
                    graphics.fill(0x7a8a6a);
                    graphics.circle(x, y - 38 * scale, 4 * scale);
                    graphics.fill(0xc0a040);
                }
                break;
            }

            case 'palm_tree': {
                // Classic desert oasis palm
                graphics.ellipse(x, y + 6, 22 * scale, 10 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                const palmH = 55 * scale;
                // Curved trunk
                graphics.moveTo(x - 4 * scale, y);
                graphics.quadraticCurveTo(x - 6 * scale, y - palmH * 0.5, x - 2 * scale, y - palmH);
                graphics.lineTo(x + 2 * scale, y - palmH);
                graphics.quadraticCurveTo(x + 2 * scale, y - palmH * 0.5, x + 4 * scale, y);
                graphics.closePath();
                graphics.fill(0x8a7a5a);

                // Trunk texture rings
                for (let i = 0; i < 8; i++) {
                    const ty = y - i * 6 * scale;
                    graphics.moveTo(x - 4 * scale, ty);
                    graphics.lineTo(x + 4 * scale, ty);
                    graphics.stroke({ color: 0x7a6a4a, width: 0.5 });
                }
                ...
                for (let i = 0; i < 8; i++) {
                    const angle = (i / 8) * Math.PI * 2;
                    const frondLen = 25 * scale;
                    const droop = Math.abs(Math.sin(angle)) * 12 * scale;
                    const endX = x + Math.cos(angle) * frondLen;
                    const endY = y - palmH + droop;

                    graphics.moveTo(x, y - palmH);
                    graphics.quadraticCurveTo(
                        x + Math.cos(angle) * frondLen * 0.5,
                        y - palmH - 5 + droop * 0.3,
                        endX, endY
                    );
                    graphics.stroke({ color: 0x4a7a3a, width: 3 * scale });
                }
                break;
            }

            case 'saguaro_cactus': {
                // Iconic tall cactus with arms
                graphics.ellipse(x, y + 5, 20 * scale, 10 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                const saguaroH = 60 * scale;
                // Main trunk
                graphics.roundRect(x - 6 * scale, y - saguaroH, 12 * scale, saguaroH, 6);
                graphics.fill(0x4a7a4a);

                // Vertical ridges
                for (let i = 0; i < 5; i++) {
                    graphics.moveTo(x - 4 * scale + i * 2 * scale, y);
                    graphics.lineTo(x - 4 * scale + i * 2 * scale, y - saguaroH + 5);
                    graphics.stroke({ color: 0x3a6a3a, width: 1 });
                }

                // Arms
                // Left arm
                graphics.moveTo(x - 6 * scale, y - saguaroH * 0.5);
                graphics.quadraticCurveTo(x - 18 * scale, y - saguaroH * 0.5, x - 18 * scale, y - saguaroH * 0.7);
                graphics.lineTo(x - 12 * scale, y - saguaroH * 0.7);
                graphics.quadraticCurveTo(x - 12 * scale, y - saguaroH * 0.55, x - 6 * scale, y - saguaroH * 0.55);
                graphics.closePath();
                graphics.fill(0x4a7a4a);

                // Right arm
                graphics.moveTo(x + 6 * scale, y - saguaroH * 0.4);
                graphics.quadraticCurveTo(x + 16 * scale, y - saguaroH * 0.4, x + 16 * scale, y - saguaroH * 0.65);
                graphics.lineTo(x + 10 * scale, y - saguaroH * 0.65);
                graphics.quadraticCurveTo(x + 10 * scale, y - saguaroH * 0.45, x + 6 * scale, y - saguaroH * 0.45);
                graphics.closePath();
                graphics.fill(0x4a7a4a);
                break;
            }
            ...
            case 'yucca_tree': {
                // Spiky desert plant
                graphics.ellipse(x, y + 5, 20 * scale, 9 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                const yuccaH = 40 * scale;
                // Short trunk
                graphics.rect(x - 4 * scale, y - yuccaH * 0.3, 8 * scale, yuccaH * 0.3);
                graphics.fill(0x7a6a5a);

                // Spiky leaves radiating from top
                for (let i = 0; i < 16; i++) {
                    const angle = (i / 16) * Math.PI * 2 + foliage.rotation;
                    const leafLen = (18 + seededRandom(foliage.id + i) * 8) * scale;
                    const endX = x + Math.cos(angle) * leafLen;
                    const endY = y - yuccaH * 0.35 + Math.sin(angle) * leafLen * 0.3;

                    graphics.moveTo(x, y - yuccaH * 0.3);
                    graphics.lineTo(endX, endY);
                    graphics.stroke({ color: 0x6a8a5a, width: 2 * scale });
                }

                // Flower stalk
                if (seededRandom(foliage.id) > 0.5) {
                    graphics.rect(x - 1.5 * scale, y - yuccaH, 3 * scale, yuccaH * 0.5);
                    graphics.fill(0x8a9a7a);
                    // White flowers
                    for (let i = 0; i < 5; i++) {
                        const fy = y - yuccaH + i * 4 * scale;
                        graphics.circle(x, fy, 3 * scale);
                        graphics.fill(0xf8f8f0);
                    }
                }
                break;
            ...
            case 'prickly_pear_cactus': {
                // Paddle-shaped cactus
                graphics.ellipse(x, y + 4, 20 * scale, 9 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });
                ...
                for (const pad of padPositions) {
                    ...
                    const psize = pad.size;

                    // Pad shape
                    graphics.ellipse(px, py, 10 * psize * scale, 8 * psize * scale);
                    graphics.fill(0x4a7a4a);

                    // Spines
                    for (let s = 0; s < 6; s++) {
                        const sx = px + (seededRandom(foliage.id + s) - 0.5) * 14 * psize * scale;
                        const sy = py + (seededRandom(foliage.id + s + 10) - 0.5) * 10 * psize * scale;
                        graphics.circle(sx, sy, 1 * scale);
                        graphics.fill(0xd0c0a0);
                    }
                }

                // Red fruits on top
                for (let i = 0; i < 2; i++) {
                    const fx = x - 2 * scale + i * 8 * scale;
                    graphics.ellipse(fx, y - 30 * scale, 3 * scale, 4 * scale);
                    graphics.fill(0xa03030);
                }
                break;
            }

            case 'joshua_tree': {
                // Distinctive spiky tree
                graphics.ellipse(x, y + 6, 24 * scale, 11 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                const joshuaH = 55 * scale;
                // Thick fibrous trunk
                graphics.rect(x - 5 * scale, y - joshuaH * 0.5, 10 * scale, joshuaH * 0.5);
                graphics.fill(0x7a6a5a);

                // Bark texture
                for (let i = 0; i < 6; i++) {
                    const ty = y - i * 5 * scale;
                    graphics.moveTo(x - 5 * scale, ty);
                    graphics.lineTo(x + 5 * scale, ty);
                    graphics.stroke({ color: 0x6a5a4a, width: 1 });
                }

                // Branching arms with spiky tufts
                const branches = [
                    { x: -12, y: -joshuaH * 0.7, angle: -0.5 },
                    { x: 14, y: -joshuaH * 0.65, angle: 0.4 },
                    { x: 0, y: -joshuaH, angle: 0 },
                    { x: -8, y: -joshuaH * 0.9, angle: -0.3 },
                ];

                for (const branch of branches) {
                    const bx = x + branch.x * scale;
                    const by = y + branch.y;

                    // Branch
                    graphics.moveTo(x, y - joshuaH * 0.5);
                    graphics.lineTo(bx, by);
                    graphics.stroke({ color: 0x7a6a5a, width: 4 * scale });
                    ...
                    for (let s = 0; s < 12; s++) {
                        ...
                        graphics.moveTo(bx, by);
                        graphics.lineTo(
                            bx + Math.cos(sangle) * slen,
                            by + Math.sin(sangle) * slen * 0.5 - 5 * scale
                        );
                        graphics.stroke({ color: 0x6a8a5a, width: 1.5 * scale });
                    }
                }
                break;
            }
            ...
            case 'doum_palm_tree': {
                ...
                const doumH = 60 * scale;
                // Main trunk that branches
                graphics.rect(x - 5 * scale, y - doumH * 0.4, 10 * scale, doumH * 0.4);
                graphics.fill(0x8a7a5a);

                // Two branch trunks
                const branchTrunks = [
                    { x: -10, endY: -doumH * 0.9 },
                    { x: 12, endY: -doumH * 0.85 },
                ];

                for (const bt of branchTrunks) {
                    graphics.moveTo(x, y - doumH * 0.4);
                    graphics.lineTo(x + bt.x * scale, y + bt.endY);
                    graphics.stroke({ color: 0x8a7a5a, width: 8 * scale });

                    // Fronds at top of each branch
                    const bx = x + bt.x * scale;
                    const by = y + bt.endY;
                    for (let i = 0; i < 6; i++) {
                        const angle = (i / 6) * Math.PI * 2;
                        const frondLen = 20 * scale;
                        const droop = Math.abs(Math.sin(angle)) * 10 * scale;
                        graphics.moveTo(bx, by);
                        graphics.quadraticCurveTo(
                            bx + Math.cos(angle) * frondLen * 0.6,
                            by - 5 + droop * 0.5,
                            bx + Math.cos(angle) * frondLen,
                            by + droop
                        );
                        graphics.stroke({ color: 0x4a7a3a, width: 2.5 * scale });
                    }
                }
                break;
            }
            ...
            case 'quiver_tree': {
                // Distinctive aloe tree
                graphics.ellipse(x, y + 6, 26 * scale, 12 * scale);
                graphics.fill({ color: 0x000000, alpha: 0.2 });

                const quiverH = 65 * scale;
                // Thick smooth trunk
                graphics.rect(x - 6 * scale, y - quiverH * 0.5, 12 * scale, quiverH * 0.5);
                graphics.fill(0xa09080);

                // Branching structure at top
                const quiverBranches = [
                    { x: -15, y: -quiverH * 0.85 },
                    { x: 0, y: -quiverH },
                    { x: 16, y: -quiverH * 0.88 },
                    { x: -8, y: -quiverH * 0.95 },
                ...
                for (const qb of quiverBranches) {
                    const bx = x + qb.x * scale;
                    const by = y + qb.y;

                    // Branch
                    graphics.moveTo(x, y - quiverH * 0.5);
                    graphics.lineTo(bx, by);
                    graphics.stroke({ color: 0xa09080, width: 5 * scale });

                    // Rosette of succulent leaves
                    for (let l = 0; l < 10; l++) {
                        const langle = (l / 10) * Math.PI * 2;
                        const llen = 12 * scale;
                        graphics.moveTo(bx, by);
                        graphics.lineTo(
                            bx + Math.cos(langle) * llen,
                            by + Math.sin(langle) * llen * 0.4 - 5 * scale
                        );
                        graphics.stroke({ color: 0x7a9a6a, width: 2 * scale });
                    }
                }

                // Yellow flowers (sometimes)
                if (seededRandom(foliage.id) > 0.6) {
                    for (const qb of quiverBranches.slice(0, 2)) {
                        graphics.circle(x + qb.x * scale, y + qb.y - 8 * scale, 5 * scale);
                        graphics.fill(0xd0c040);
                    }
                }
                break;
            }
